...
3. Anonymous session middleware: httpOnly cookie issued on first visit; every query and artifact access scoped to that session.
...
6. Artifact serving: session-checked `.pdf`/`.tex` file responses plus inline `tex` in the status payload for the frontend's copy pane.
...
**Do not:** call the Anthropic API outside `core`, add auth or queues, or serve artifacts without session checks.
