...
// so a stale/partial order never drops an item. A key repeated in `order`
// is only honored once, at its first occurrence, so the item can't render
// twice in the live preview.
